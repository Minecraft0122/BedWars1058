...
public class QuitAndTeleportListener implements Listener {
    ...
    static void removeArenaPlayerOnQuit(IArena arena, Player player, PlayerQuitEvent.QuitReason reason) {
        ...
        if (!finalAbandonment && BedWars.plugin != null
                ...
                && ReJoin.getPlayer(player) == null) {
            /* A player whose team/bed was already cleaned up cannot receive a
             * reconnect lease. Record that departure now instead of silently
             * allowing a disconnect to bypass the abandonment policy. */
            BedWars.plugin.getDisciplineService().markAbandonment(
                    player.getUniqueId(), arena, "DISCONNECT_NO_REJOIN");
        }
        if (finalAbandonment) {
