...
public class ReJoin {
    ...
    public ReJoin(Player player, IArena arena, ITeam bwt, List<ShopCache.CachedItem> cachedArmor) {
        ...
        this.arena = arena;
        long rejoinSeconds = rejoinWindowSeconds();
    ...
    /** Keep local expiry tasks and lobby-published deadlines identical. */
    public static long rejoinWindowSeconds() {
        return Math.min(86_400L, Math.max(1L, BedWars.config.getYml().getLong(
